...
musl instead of relying on the prebuilt glibc release. `Dockerfile.alpine-podman`
...
## Further size trimming in `Dockerfile.alpine-podman`

Beyond the source build itself, the final image drops two things that
turned out to be unnecessary weight (~918MB -> ~757MB):

- `gnupg` is not installed at all — the image installs Podman directly via
...
This repository now ships only the Podman-based image. See the main
`README.md` for runtime behavior, build limitations, security notes, and
registry usage.
